...
class MiMoFileAsrEngine(
    ...
    private fun isAudioUnderstandingModel(model: String): Boolean =
        model.equals(MODEL_AUDIO_UNDERSTANDING, ignoreCase = true) ||
    ...
    override suspend fun recognize(pcm: ByteArray) {
        val audio = if (prefs.uploadAudioCompressionEnabled && uploadAudioEncodingSpec != null) {
    ...
    private fun buildRequestBody(
        ...
        val mime = when (audio.container) {
            UploadAudioContainer.WAV -> "audio/wav"
            else -> "audio/mpeg"
        }
        ...
        if (!isAuModel) {
            body.put("asr_options", JSONObject().apply {
