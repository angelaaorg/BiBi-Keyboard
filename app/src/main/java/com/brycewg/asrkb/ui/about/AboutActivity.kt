...
class AboutActivity : BaseActivity() {
        ...
        btnToggle.setOnClickListener { v ->
            ...
            try {
                if (DebugLogManager.isRecording()) {
                    ...
                    Toast.makeText(
                        this,
                        R.string.toast_debug_recording_stopped,
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
                    ...
                    Toast.makeText(
                        this,
                        R.string.toast_debug_recording_started,
                        Toast.LENGTH_SHORT
                    ).show()
                    try {
                        val overlayOk = try {
            ...
            try {
                ...
                when (result) {
                        ...
                        try {
                            startActivity(
                                Intent.createChooser(
                                    result.intent,
                                    getString(R.string.btn_debug_export)
                                )
                            )
                        } catch (e: Throwable) {
                            ...
                            Toast.makeText(
                                this,
                                R.string.toast_debug_export_failed,
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
                    is DebugLogManager.ShareIntentResult.Error -> {
                        when (result.error) {
                            DebugLogManager.ShareError.RecordingActive -> Toast.makeText(
                                this,
                                R.string.toast_debug_stop_before_export,
                                Toast.LENGTH_SHORT
                            ).show()
                            DebugLogManager.ShareError.NoLog -> Toast.makeText(
                                this,
                                R.string.toast_debug_no_log,
                                Toast.LENGTH_SHORT
                            ).show()
                            DebugLogManager.ShareError.Failed -> Toast.makeText(
                                this,
                                R.string.toast_debug_export_failed,
                                Toast.LENGTH_SHORT
                            ).show()
    ...
    private fun renderUsageStats() {
        ...
        val vendorPairs = stats.perVendor.map {
            it.key to it.value
     ...
     * 从 assets 目录读取文件内容
     */
    private fun readAssetFile(fileName: String): String = try {
